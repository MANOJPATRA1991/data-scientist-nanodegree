...
def get_movie_names(movie_ids, movies_df):
    ...
    movie_names_list = list(movies_df[movies_df['movie_id'].isin(movie_ids)]['movie'])
...
def find_similar_movies(movie_id, movies_df):
    ...
    # Find the row of the movie_id
    movie_idx = np.where(movies_df['movie_id'] == movie_id)[0][0]

    # Find the movies which are of the highest similarity to the movie with movie_id
